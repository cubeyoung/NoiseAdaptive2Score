...
class GaussianModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.lr = self.hr.numpy() + np.random.normal(size = self.hr.shape,scale = self.phi_s)
    ...
    def forward(self):
        ...
        self.variance = torch.from_numpy((self.phi_s)**2).to(self.device,dtype = torch.float32)
        self.score = (self.variance)*self.netf(self.lr,0)[0]
        self.recon = self.score+self.lr
